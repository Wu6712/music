...
router.get('/picSrc/:fileName', (req, res) => {
  ...
  try {
    ...
    let picUrl = './pic/banner/' + fileName
...
router.post('/upload', (req, res) => {
  ...
  filePath = './pic/banner/' + fileObj.name;
  fileObj.mv(filePath, (err) => {
    ...
    let sql = $sql.banner.add;
    let params = [req.files.file.name, "http://localhost:8080/api/banner/picSrc/" + req.files.file.name]  // 这边的数组参数与上边的"?"一一映射
...
router.get('/getall', (req, res, next) => {
  let result = fs.readdirSync('./pic/banner')
  res.json(result)
})

//模糊查询
router.get("/queryFuzzy", (req, res) => {
  let sql = $sql.banner.queryByName
...
//查询数据
router.get('/queryLimit', (req, res) => {
  let sql = $sql.banner.queryPage; //sql查询语句：查找banner表中所有数据
  let params = [req.query.pageNum,req.query.pageSize]
...
router.get('/queryLimit2', function (req, res) {
  ...
  con.query(sql, params, function (err, result) {
      if (err) {
          ...
          let sqlTotal = 'select count(*) as total from banner' //as更换名称
...
router.get('/query', (req, res) => {
  let sql = $sql.banner.query; //sql查询语句：查找banner表中所有数据
