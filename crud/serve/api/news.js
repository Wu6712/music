...
router.post('/upload', (req, res) => {
  ...
  filePath = './pic/news/' + fileObj.name;
  fileObj.mv(filePath, (err) => {
    ...
    let sql = $sql.news.add;
    let params = [req.body.name, "http://localhost:8080/api/news/picSrc/" + req.files.file.name,req.body.content]  // 这边的数组参数与上边的"?"一一映射
...
router.get('/getall', (req, res, next) => {
  let result = fs.readdirSync('./pic/news')
  res.json(result)
})

//模糊查询
router.get("/queryFuzzy", (req, res) => {
  let sql = $sql.news.queryByName
...
router.get('/query', (req, res) => {
  let sql = $sql.news.query; //sql查询语句：查找banner表中所有数据
...
router.post('/add', (req, res) => {
  let sql = $sql.news.add;
  let params = [req.query.name, req.query.link, req.query.content]  // 这边的数组参数与上边的"?"一一映射
  con.query(sql, params, (err, result) => {
    try {
      res.send({
        code:200,
...
router.put('/mod', (req, res) => {
  let sql = $sql.news.mod;
  let params = [req.query.name, req.query.link,req.query.content, req.query.id]
